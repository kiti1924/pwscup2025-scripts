...
def configure_stdio(encoding: str | None) -> None:
    target = encoding
    if not target:
        try:
            target = locale.getpreferredencoding(False) or None
        except Exception:
            target = None
    if not target:
        return
    ...
    for stream in (sys.stdout, sys.stderr):
        try:
            ...
            if current and current.lower() == lowered:
                stream.reconfigure(errors='backslashreplace')
            else:
                stream.reconfigure(encoding=target, errors='backslashreplace')
...
def main():
    ...
    ap.add_argument("--input", "-i", dest="input_csv", default="input.csv", help="Input CSV file")
    ap.add_argument("json_spec", help="JSON schema file")
    ap.add_argument("--output", "--out", "-o", dest="output_csv", default="output.csv", help="Output CSV file")
    ap.add_argument("--report", "-r", default=None, help="Optional CSV to store numeric fix details")
    ap.add_argument("--input-encoding", default=None, help="Encoding used to read the input CSV")
    ap.add_argument("--output-encoding", default=None, help="Encoding used to write CSV outputs")
    ap.add_argument("--stdio-encoding", default=None, help="Encoding used for stdout/stderr text")
    args = ap.parse_args()

    configure_stdio(args.stdio_encoding)
    ...
    read_csv_kwargs = dict(dtype=str, keep_default_na=False)
    if args.input_encoding:
        read_csv_kwargs["encoding"] = args.input_encoding
    try:
        df = pd.read_csv(csv_path, **read_csv_kwargs)
    except UnicodeDecodeError as e:
        print(f"CSV decoding error: {e}. Specify --input-encoding with the correct codec.", file=sys.stderr)
        sys.exit(1)
    ...
    if args.output_encoding:
        to_csv_kwargs["encoding"] = args.output_encoding
    ...
    if num_fixes:
        ...
        if report_path:
            ...
            rep_df.to_csv(report_path, **to_csv_kwargs)
