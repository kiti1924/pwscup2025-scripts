...
def eval_Ci_df_utility(Bi_df:pd.DataFrame, Ci_df:pd.DataFrame,
                       print_details:bool=False, output_file:str=None)->float:
    ...
    result = (
        f"stats_diff max_abs: {stats_diff_max_abs}\n"
        f"LR_asthma_diff max_abs: {LR_asthma_diff_max_abs}\n"
...
if __name__ == "__main__":
    ...
    ap.add_argument("-o", "--output", help="[optional] 結果を出力するtxtファイルパス", default=None)
    args = ap.parse_args()
    ...
    try:
        ...
        eval_Ci_df_utility(Bi_df, Ci_df, print_details=args.print_details, output_file=args.output)
        ...
        if args.force:
            print("採点を強行します")
            eval_Ci_utility(args.Bi_csv, args.Ci_csv, print_details=args.print_details, output_file=args.output)
